...
import {
  getStacks, stackCreate, stackUp, stackDown, stackSync, stackSubmit, stackFold, stackAbsorb,
  type Stack, type StackBranch,
} from "../lib/stack.ts"
import { refreshBranches, refreshStatus } from "../hooks/use-git.ts"
import { InputDialog, ConfirmDialog, SelectDialog } from "../components/dialog.tsx"
import { getDiffBetween } from "../lib/git.ts"
...
function prStatusColor(status?: string): string {
  switch (status) {
    case "open":   return C.prOpen
    case "draft":  return C.prDraft
...
export function StacksView() {
  ...
  async function loadStacks() {
    setLoading(true)
    try {
      const s = await getStacks(state.repoRoot)
      setStacks(s)
      ...
      if (firstBranch && firstStack) {
        ...
        const diff = await getDiffBetween(parent, firstBranch.name, state.repoRoot)
        setDiffContent(diff)
      }
    } catch (err) {
  ...
  async function loadBranchDiff(stack: Stack, branch: StackBranch) {
    try {
      ...
      const diff = await getDiffBetween(parent, branch.name, state.repoRoot)
      setDiffContent(diff)
    } catch {
      setDiffContent("")
    }
  ...
  useKeyboard(async (key) => {
    ...
    if (key.name === "j" || key.name === "down") {
      ...
      key.preventDefault()
      try {
        const target = await stackUp(state.repoRoot)
        if (target) {
      ...
      key.preventDefault()
      try {
        const target = await stackDown(state.repoRoot)
        if (target) {
          showToast("info", `Moved to ${target}`)
          await loadStacks()
        }
      ...
      key.preventDefault()
      try {
        const ok = await stackAbsorb(state.repoRoot)
        if (ok) {
    ...
    <box flexGrow={1} flexDirection="row">
      ...
      >
                  ...
                  <box flexDirection="column" paddingTop={1} paddingLeft={1}>
                    ...
                    <For each={[...stack.branches].reverse()}>
                      {(branch) => {
                        ...
                        // Find linked PR
                        const pr = () => state.prs.find(p => p.headRefName === branch.name)
                        const ci = () => {
                          const p = pr()
                          if (!p) return null
                          const checks = p.checks
                          if (checks.some(c => c.conclusion === "FAILURE")) return "failing"
                          if (checks.some(c => c.status === "IN_PROGRESS")) return "pending"
                        ...
                        return (
                          ...
                          >
                            <box flexDirection="row" gap={1}>
                              ...
                              {/* CI status */}
                              <Show when={ci()}>
                                <text fg={ci() === "passing" ? C.ciPass : ci() === "failing" ? C.ciFail : C.ciPending}>
        ...
        <box height={1} paddingLeft={1} border={["top"]} borderColor={C.border}>
          <text fg={C.dim}>
            ...
            <span style={{ fg: "#58a6ff" }}>s</span> sync ·{" "}
            <span style={{ fg: "#58a6ff" }}>p</span> submit ·{" "}
            <span style={{ fg: "#58a6ff" }}>a</span> absorb ·{" "}
      ...
      <Show when={showCreate()}>
        <InputDialog
          ...
          onSubmit={async (name) => {
            setShowCreate(false)
            try {
              await stackCreate(name, undefined, state.repoRoot)
              await Promise.all([loadStacks(), refreshBranches()])
      ...
      <Show when={showSync()}>
        <ConfirmDialog
          ...
          onConfirm={async () => {
            setShowSync(false)
            try {
              await stackSync(state.repoRoot)
              await Promise.all([loadStacks(), refreshStatus()])
      ...
      <Show when={showSubmit() && currentStack()}>
        <SelectDialog
          ...
          options={[
            { label: "Submit as ready PRs", description: "Create/update PRs for all branches", value: "submit" },
            { label: "Submit as draft PRs", description: "Create draft PRs (not ready for review)", value: "draft" },
          ]}
          onSelect={async (mode: string) => {
            ...
            if (!stack) return
            try {
              await stackSubmit(stack.id, { draft: mode === "draft" }, state.repoRoot)
              await loadStacks()
      ...
      <Show when={showFold() && currentBranch()}>
        <ConfirmDialog
          ...
          onConfirm={async () => {
            setShowFold(false)
            try {
              await stackFold(state.repoRoot)
              await Promise.all([loadStacks(), refreshBranches()])
