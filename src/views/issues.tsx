...
import { listIssues, getIssueComments, commentOnIssue, closeIssue, reopenIssue, type Issue, type IssueComment } from "../lib/gh.ts"
import { refreshIssues } from "../hooks/use-gh.ts"
  ...
  return (
    <box flexGrow={1} flexDirection="row">
      ...
      >
        ...
        <box height={1} paddingLeft={1} border={["top"]} borderColor={C.border}>
          <text fg={C.dim}>
            ...
            <span style={{ fg: "#58a6ff" }}>x</span> close ·{" "}
      ...
      <box
        ...
        title={selectedIssue() ? `#${selectedIssue()!.number}: ${selectedIssue()!.title}` : "issue detail"}
      >
        <Show when={selectedIssue()}>
          <scrollbox flexGrow={1} scrollbarOptions={{ visible: true }} paddingLeft={1} paddingRight={1} paddingTop={1}>
            {/* Issue body */}
            ...
            <Show when={comments().length > 0}>
              <box paddingTop={1}>
                <text fg={C.dim}>── {comments().length} comment{comments().length !== 1 ? "s" : ""} ──</text>
              </box>
              <For each={comments()}>
                {(comment) => (
                  <box
