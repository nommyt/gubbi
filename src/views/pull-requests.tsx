...
import { state, setState, showToast } from "../lib/store.ts"
import { listPRs, getPRDiff, mergePR, reviewPR, closePR, type PullRequest } from "../lib/gh.ts"
import { refreshPRs } from "../hooks/use-gh.ts"
import { checkoutPR } from "../lib/gh.ts"
import { exec } from "../lib/shell.ts"
import { SelectDialog, InputDialog, ConfirmDialog } from "../components/dialog.tsx"
...
function checksIcon(pr: PullRequest): { icon: string; color: string } {
  ...
  if (checks.some(c => c.status === "IN_PROGRESS" || c.status === "QUEUED")) return { icon: "●", color: C.ciPending }
  if (checks.every(c => c.conclusion === "SUCCESS" || c.conclusion === "SKIPPED")) return { icon: "✓", color: C.ciPass }
                  ...
                  return (
                    ...
                    >
                      ...
                      <box flexDirection="row" gap={1} paddingLeft={2}>
                        ...
                        <text fg={C.dim}>+{pr.additions} -{pr.deletions}</text>
                        <For each={pr.labels.slice(0, 3)}>
                          {(label) => (
                            <text fg={C.label}>[{label}]</text>
                          )}
                        </For>
                      </box>
                    </box>
      ...
      <Show when={showMerge() && selectedPR()}>
        <SelectDialog
          ...
          onSelect={async (method: string) => {
            ...
            if (!pr) return
            try {
              await mergePR(pr.number, method as "merge" | "squash" | "rebase", { deleteAfterMerge: true })
              await loadPRs()
      ...
      <Show when={showReview()}>
        <SelectDialog
          ...
          options={[
            ...
            { label: "Request changes", description: "Request changes to the PR", value: "request-changes" },
      ...
      <Show when={showComment()}>
        <InputDialog
          ...
          onSubmit={async (body) => {
            ...
            if (!pr) return
            try {
              const { commentOnPR } = await import("../lib/gh.ts")
