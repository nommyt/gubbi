...
import { getRemotes, fetch, getWorktrees, addWorktree, removeWorktree, type WorktreeEntry } from "../lib/git.ts"
import { refreshStatus } from "../hooks/use-git.ts"
import { InputDialog, ConfirmDialog } from "../components/dialog.tsx"
import type { RemoteEntry } from "../lib/parser.ts"
...
export function RemotesView() {
  ...
  async function loadData() {
    ...
    try {
      const [r, w] = await Promise.all([
        getRemotes(state.repoRoot),
        getWorktrees(state.repoRoot),
      ])
      setRemotes(r)
      setWorktrees(w)
    } catch (err) {
  ...
  useKeyboard(async (key) => {
    ...
    if (key.name === "j" || key.name === "down") {
      ...
      if (!remote) return
      try {
        await fetch(remote.name, { prune: true }, state.repoRoot)
        await refreshStatus()
              ...
              {(remote, i) => {
                ...
                return (
                  ...
                  >
                    ...
                    <text fg={C.url} paddingLeft={2}>{remote.fetchUrl}</text>
                  </box>
                )
              }}
            </For>
          ...
          <scrollbox flexGrow={1}>
            <For each={worktrees()}>
              {(wt, i) => {
                ...
                const isCurrent = () => wt.path === state.repoRoot
                return (
                  <box
                    flexDirection="column"
                    paddingLeft={1}
      ...
      <Show when={showAddWorktree()}>
        <InputDialog
          ...
          onSubmit={async (path) => {
            ...
            try {
              await addWorktree(path, state.currentBranch, state.repoRoot)
              await loadData()
      ...
      <Show when={showRemoveWorktree()}>
        <ConfirmDialog
          ...
          onConfirm={async () => {
            ...
            if (!wt) return
            try {
              await removeWorktree(wt.path, false, state.repoRoot)
              await loadData()
