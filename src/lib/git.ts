...
export async function getGraphLog(opts: { count?: number; all?: boolean } = {}, cwd?: string): Promise<string> {
  const args = [
    "log",
    "--graph",
    "--oneline",
    "--decorate",
    `--max-count=${opts.count ?? 100}`,
  ]
  if (opts.all) args.push("--all")
  const r = await exec(GIT, args, { cwd })
  return r.stdout
...
export async function getFileLog(path: string, opts: { count?: number; follow?: boolean } = {}, cwd?: string): Promise<LogEntry[]> {
  const args = [
    "log",
    `--format=${LOG_FORMAT}`,
    `--max-count=${opts.count ?? 100}`,
...
export async function commit(message: string, opts: CommitOptions = {}, cwd?: string): Promise<void> {
...
export async function deleteRemoteBranch(remote: string, name: string, cwd?: string): Promise<void> {
...
export async function fetch(remote?: string, opts: { prune?: boolean } = {}, cwd?: string): Promise<void> {
  const args = ["fetch"]
