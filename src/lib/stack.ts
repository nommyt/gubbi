...
import { getConfig, setConfig, unsetConfig, getCurrentBranch, createBranch, rebaseOnto, push, getLog } from "./git.ts"
  ...
  for (let i = 0; i < stack.branches.length; i++) {
    ...
    if (prCheckR.exitCode === 0) {
      ...
      const prArgs = [
        ...
        "--base", base,
        "--title", subject,
        "--body", `Part of stack: ${stack.branches.map(b => `\`${b.name}\``).join(" → ")}`,
      ]
