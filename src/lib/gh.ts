...
export interface PRCheck {
  ...
  conclusion: "SUCCESS" | "FAILURE" | "SKIPPED" | "CANCELLED" | "TIMED_OUT" | "ACTION_REQUIRED" | null
  detailsUrl: string
}
...
const PR_FIELDS = [
  ...
  "headRefName", "baseRefName", "body", "labels",
  "reviewDecision", "additions", "deletions", "changedFiles",
  "createdAt", "updatedAt", "url", "mergeable", "mergeStateStatus",
  "statusCheckRollup",
].join(",")
...
const ISSUE_FIELDS = [
  ...
  "labels", "assignees", "comments", "reactions",
...
function mapIssue(raw: Record<string, unknown>): Issue {
  ...
  const author = typeof raw.author === "object" && raw.author !== null
    ? String((raw.author as Record<string, unknown>).login ?? "")
    : String(raw.author ?? "")
  const comments = typeof raw.comments === "number"
    ? raw.comments
...
const RUN_FIELDS = [
  "databaseId", "name", "workflowName", "status", "conclusion",
  "headBranch", "event", "headSha", "createdAt", "updatedAt", "url", "attempt",
].join(",")
...
export async function getRepoInfo(): Promise<RepoInfo | null> {
  const r = await exec(GH, [
    "repo", "view",
    "--json", "name,owner,nameWithOwner,description,defaultBranchRef,isPrivate,stargazerCount,forkCount,url",
  ])
  if (r.exitCode !== 0) return null
  try {
