...
import { state, type ViewName } from "../lib/store.ts"
...
const VIEW_HINTS: Record<ViewName, KeyHint[]> = {
  smartlog: [
    { key: "j/k", label: "nav" },
...
export function StatusBar() {
  const hints = () => VIEW_HINTS[state.currentView] ?? []
  ...
  return (
    ...
    >
      ...
      <Show when={latestToast()}>
        <text>
          <span style={{
            fg: latestToast()?.type === "error" ? C.error
              : latestToast()?.type === "success" ? C.success
              : latestToast()?.type === "warning" ? C.warning
              : C.info
          }}>
            {latestToast()?.message}
          </span>
      ...
      {/* Current view name */}
      <text fg={C.activeView}>
        {VIEWS_DISPLAY[state.currentView]}
      </text>
    </box>
  )
}

import { VIEWS } from "../lib/store.ts"

const VIEWS_DISPLAY: Record<ViewName, string> = Object.fromEntries(
