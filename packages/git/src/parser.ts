...
export const LOG_FORMAT = `%H${LOG_SEP}%h${LOG_SEP}%an${LOG_SEP}%ae${LOG_SEP}%ai${LOG_SEP}%ci${LOG_SEP}%ar${LOG_SEP}%s${LOG_SEP}%D${LOG_SEP}%P${LOG_SEP}%G?${LOG_RECORD_SEP}`
...
export function parseBranches(output: string, currentBranch: string): BranchEntry[] {
	...
	for (const line of lines) {
		const [fullRef = "", hash = "", subject = "", date = "", upstream = "", track = ""] =
			line.split("\x01")
