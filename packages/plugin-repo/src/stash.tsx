...
export function StashView() {
	...
	async function loadDiff(index: number) {
		const diff = await getStashDiff(index, state.git.repoRoot)
		setDiffContent(diff)
	}
	...
	useKeyboard(async (key) => {
		...
		if (key.name === "j" || key.name === "down") {
			...
			key.preventDefault()
			try {
				await stashPop(entry.index, state.git.repoRoot)
				await Promise.all([loadEntries(), gitService.refreshStatus()])
			...
			<Show when={showApply() && selectedEntry()}>
				<SelectDialog
					...
					options={[
						{
							...
							description: "Apply and keep the stash entry",
							value: "apply",
						},
						{
							label: "Pop (remove stash)",
							description: "Apply and remove the stash entry",
							value: "pop",
						},
					]}
					onSelect={async (action: string) => {
						...
						if (!entry) return
						try {
							if (action === "pop") await stashPop(entry.index, state.git.repoRoot)
							else await stashApply(entry.index, state.git.repoRoot)
							await Promise.all([loadEntries(), gitService.refreshStatus()])
			...
			<Show when={showDrop() && selectedEntry()}>
				<ConfirmDialog
					...
					onConfirm={async () => {
						...
						if (!entry) return
						try {
							await stashDrop(entry.index, state.git.repoRoot)
							await loadEntries()
